...
# ---> ENSURE THIS EXACT BLOCK IS IN YOUR FRONTEND APP.PY <---
@app.route('/register')
def register_page():
...
@app.route('/main')
def home():
    try: impact = requests.get(f"{B_URL}/impact").json().get('fci_community_impact', {})
    except: impact = {"total_items_reused": 0, "co2_offset_kg": 0}
...
def search():
    ...
    try: items = requests.get(f"{B_URL}/items").json()
    except: items = []
